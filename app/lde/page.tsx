...
export default function LDE() {
    ...
    async function remove(e: any) {
        e.preventDefault()
        try {
            await api.removeLDE(removeItem!)
            setRemoveItem(0)
            updatePage()
        } catch (error: any) {
            alerts.showErrorAlert(error.response.data)
    ...
    async function obterItem(e: any) {
        ...
        try {
            ...
            const res = await api.obterLDE(value)
    ...
    return (
        <div className="flex flex-col w-full items-center align-between">
            ...
            {loading ? <Loading /> : <>
                <ContentTitle>Lista duplamente encadeada</ContentTitle>
                <OperationsContainer>
                ...
                <div className="flex justify-center my-16 flex-wrap w-5/6">
                    {list.length !== 0 ?
                        list.map((item: any) => <ListItemLDE selectedItem={item.conteudo === itemObtido ? true : false}>{item}</ListItemLDE>)
